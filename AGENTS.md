...
### ⚠️ 刷新必须走 HTTP
TCP `0xFF` 已废弃——`TurnOnDisplay()` 阻塞 ~15 秒（`ReadBusy()` 轮询），在 TCP handler 中直接调用会导致看门狗复位。刷新统一用 `POST /display/refresh`，该接口先发 HTTP 响应再调用 `TurnOnDisplay()`，在 `delay()` yield 期间逐步发送响应。
...
## Verified Pitfalls & Lessons

### 1. 不要手动逐字节循环写 SPI
❌ 手动 `for (i=0; i<7500; i++) EPD_4IN2B_V2_SendData(buf[i])` 没有 `yield()`，看门狗复位。
✅ 始终使用 `EPD_4IN2B_V2_SendHalfBimage()` / `EPD_4IN2B_V2_SendHalfRYimage()`。

### 2. ESP8266WebServer POST body 限制
`server.arg("plain")` 内部用 `String` 存储 body。7500 字节二进制数据导致 String 扩容至 8K~16K，结合其他分配后超出 ~14K 空闲堆 → 崩溃。
✅ 图像数据走 TCP :81，绕过 HTTP body 缓冲。

### 3. TurnOnDisplay 阻塞 15 秒
`EPD_4IN2B_V2_TurnOnDisplay()` 内部 `ReadBusy()` 轮询 BUSY 引脚，阻塞约 15 秒。
✅ HTTP handler 先发响应再调用 TurnOnDisplay（`delay()` 期间自动 yield 发送响应）。
❌ TCP handler 中调用会卡死看门狗。

### 4. 半缓冲区策略
- 黑色层：TCP 0x00 缓冲上半到 `g_ImgBuf`，TCP 0x01 时先发上半再读下半一起发。
- 红色层：同理。
- 缓冲后立即发送到 EPD RAM，不等待全部 4 块（节省内存）。
- 全部发送完成后 HTTP `/display/refresh` 触发物理刷新。

### 5. 数据格式验证
- 位图: 1bpp, row-major, MSB first
- bit=1 → 白色, bit=0 → 该层颜色 (黑/红)
- 上位机负责格式转换，无需在下位机解码
- 参考实现: `test_epd.py`
